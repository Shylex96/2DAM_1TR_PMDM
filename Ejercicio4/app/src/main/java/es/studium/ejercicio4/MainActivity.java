...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View v){
        EditText placeholder = findViewById(R.id.editTextTextEmailAddress);
        if (placeholder.length()==0) {
            Toast.makeText(MainActivity.this, "No escribiste nada", Toast.LENGTH_SHORT).show();
        } else if (!placeholder.getText().toString().contains("@")){
            Toast.makeText(MainActivity.this, "No es un email válido", Toast.LENGTH_SHORT).show();
        } else {
